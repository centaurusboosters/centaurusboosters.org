  ...
  return (
    <div style={{ position: 'relative', height: 680, overflow: 'hidden', background: 'linear-gradient(125deg,#0b1838 0%,#14275e 55%,#0b1838 100%)' }}>
      ...
      <div id="spot-golf" className="spot" style={spotStyle(active === 0 ? 0 : -100, active === 0)}>
        <div style={badgeStyle}>NOW REGISTERING · {tournament.edition.toUpperCase()}</div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 22, marginTop: 24, color: '#cdd6ee', fontWeight: 700, fontSize: 17 }}>
          <span>{tournament.date.toUpperCase()}</span><span className="hero-sep" style={{ color: '#5e74b3' }}>|</span><span>{tournament.time.toUpperCase()}</span><span className="hero-sep" style={{ color: '#5e74b3' }}>|</span><span>{tournament.venue.toUpperCase()}</span>
        </div>
      ...
      <div id="spot-mission" className="spot" style={spotStyle(active === 1 ? 0 : 100, active === 1)}>
        <div style={{ ...badgeStyle, background: 'rgba(255,255,255,.14)' }}>{mission.badge.toUpperCase()}</div>
        <h1 style={{ ...heroHeadingStyle, maxWidth: 700 }}>{mission.headline_line1}<br />{mission.headline_line2} <span style={{ color: '#d8242f' }}>{mission.headline_accent}</span></h1>
        <p style={{ color: '#cdd6ee', fontSize: 19, lineHeight: 1.5, maxWidth: 540, margin: '24px 0 0', fontWeight: 500 }}>{mission.body}</p>
        <div className="hero-cta" style={{ display: 'flex', gap: 14, marginTop: 34 }}>
...
function dotStyle(active) {
  return {
    ...
    height: 5,
    borderRadius: 3,
    background: active ? '#d8242f' : 'rgba(255,255,255,.3)',
