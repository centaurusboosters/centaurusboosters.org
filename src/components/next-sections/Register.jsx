  ...
  return (
    <div id="register" style={{ padding: '84px 6vw', background: '#f3f5fb', scrollMarginTop: 72 }}>
      <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 30px' }}>
        ...
        <h2 style={{ fontFamily: "'Anton',sans-serif", fontSize: 54, color: '#0b1838', margin: '12px 0 8px' }}>{tournament.register_headline}</h2>
        <p style={{ color: '#55585f', fontSize: 16, lineHeight: 1.55, margin: 0 }}>{tournament.register_intro} Questions? Contact {contacts.players.name} at <a href={`mailto:${contacts.players.email}`}>{contacts.players.email}</a>.</p>
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '12px 32px', background: '#0b1838', borderRadius: 6, padding: '18px 28px', margin: '0 auto 28px', maxWidth: 680 }}>
        <span style={detailStyle}>⏰ ARRIVE BY {tournament.arrive_by}</span>
        <span style={{ color: 'rgba(255,255,255,.2)', fontSize: 14 }}>|</span>
        <span style={detailStyle}>🏌 {tournament.time.toUpperCase()}</span>
        <span style={{ color: 'rgba(255,255,255,.2)', fontSize: 14 }}>|</span>
