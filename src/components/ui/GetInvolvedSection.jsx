...
export default function GetInvolvedSection({ items, formUrls }) {
  return (
    <div id="getinvolved" style={{ padding: '84px 6vw', background: '#0b1838', scrollMarginTop: '72px' }}>
      <div style={{ textAlign: 'center', marginBottom: '38px' }}>
        ...
        <h2 style={{ fontFamily: "'Anton',sans-serif", fontSize: '46px', color: '#fff', margin: '12px 0 0' }}>
          WAYS TO BACK THE WARRIORS
        </h2>
      </div>
        ...
        {items.map((item, i) =>
          item.coming_soon ? (
            <div key={i} style={cardStyle}>
              ...
              <p style={{ color: '#aebbe0', fontSize: '15px', lineHeight: 1.55 }}>{item.description}</p>
              <span style={{ display: 'inline-block', color: '#7fa0ff', fontWeight: 800, fontSize: '13px', letterSpacing: '.06em', border: '1px solid rgba(127,160,255,.35)', padding: '5px 12px', borderRadius: '3px' }}>
                COMING SOON
              </span>
            </div>
          ) : (
            ...
            >
              <div style={{ color: '#fff', fontFamily: "'Anton',sans-serif", fontSize: '28px' }}>{item.title}</div>
              <p style={{ color: '#aebbe0', fontSize: '15px', lineHeight: 1.55 }}>{item.description}</p>
              <span style={{ color: '#7fa0ff', fontWeight: 800, fontSize: '14px' }}>{item.link_label}</span>
            </button>
          )
        )}
      </div>
