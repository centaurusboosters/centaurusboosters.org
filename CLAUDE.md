...
`page.tournament.enabled` (boolean in `home.json`, toggled via TinaCMS) controls whether the golf tournament sections render at all. When `false`, tournament sections (`GolfEventClient`, `CourseClient`, `RegisterClient`, related footer content, stat band) are hidden entirely. This is the off-season mode, and stays a hard override regardless of the date-driven visibility below.
...
- Tournament visibility uses two independent `resolveSchedule()` windows in `HomePage.jsx`, both keyed off `page.tournament.enabled` plus a separate date field, so "is the event still relevant" and "can people still register" can diverge:
  - `page.tournament.event_date` (optional datetime) drives `showTournament` — the whole section (`GolfEventClient`, `CourseClient`, `RegisterClient`, related footer content, stat band, Nav links, Hero's golf slide) auto-hides starting the day *after* the event, same as the manual `enabled` toggle. Deliberately a separate field from the free-text `date`/`time`/`venue` display strings, so those never need migrating.
  - `page.tournament.registration_closes` (optional datetime) drives `showRegisterCta` (`showTournament && registrationSchedule.active`) — it does **not** hide the section, only the register button. Once it passes, the Register button in both Hero and `RegisterClient` is replaced by `page.tournament.registration_closed_message`, and the countdown `.deadline-note` (`golfDeadlineSentence()`, shown only within `GOLF_COUNTDOWN_WINDOW_DAYS`/45 of the deadline) goes quiet. The Hero's Sponsor button (uses `site.sponsor_cta.cta_label` + `forms.sponsorship`) is paired with the Register button and disappears alongside it — the standalone `SponsorCTAClient` (`#sponsor`) section is unaffected and always renders.
  - Leaving either date blank means that deadline never arrives, so `enabled` stays the only switch for that half.
