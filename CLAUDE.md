...
- **Two separate Blob stores exist**, one per environment scope: `centaurus-boosters-media` (connected to Production + Preview) and `centaurus-boosters-media-dev` (connected to Development only). This keeps local testing — including delete testing — from touching real production media. Each store injects its own `BLOB_READ_WRITE_TOKEN` scoped to its connected environment(s); there are two separate `BLOB_READ_WRITE_TOKEN` rows in `vercel env ls`, not one shared across all three. If you ever need to change which environments a store is connected to, use the Storage tab's connection UI (disconnect/reconnect with the right environment checkboxes) rather than `vercel env rm` on the derived token — removing the env var directly does not update the store's connection record, so the two can drift out of sync (learned the hard way: `vercel env rm BLOB_READ_WRITE_TOKEN development` deleted the token for all three environments at once, since it was originally one shared connection).
